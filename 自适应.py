...
请从以下选项中选择：
1. "simple" - 简单问题，可以直接回答或需要少量信息
2. "complex" - 复杂问题，需要深入分析和多源信息

请只返回复杂度级别（simple或complex），不要添加其他解释。
"""
            response = requests.post(
                f"{self.base_url}/api/generate",
                json={
...
class EnhancedMedicalRAGSystem:
    ...
    def adaptive_retrieve(self, query: str, top_k: int = 5) -> List[Dict]:
        """自适应RAG检索方法"""
        # 第一步：评估查询复杂度
        complexity = self.ollama.assess_query_complexity(query)
        print(f"🔍 查询复杂度评估: {complexity}")

        # 第二步：根据复杂度自适应调整检索策略
        if complexity == "simple":
            # 简单查询：使用快速检索，较少文档
            retrieved_docs = self._fast_retrieve(query, top_k=min(top_k, 3))
            print("🚀 使用快速检索策略")
        else:
            # 复杂查询：使用深度检索，更多文档和混合方法
            retrieved_docs = self._deep_retrieve(query, top_k=top_k * 2)  # 检索更多文档
            print("🔬 使用深度检索策略")
    ...
    def _fast_retrieve(self, query: str, top_k: int) -> List[Dict]:
        """快速检索 - 针对简单问题"""
        # 主要使用TF-IDF快速检索
        results = self._tfidf_retrieve(query, top_k * 2)

        # 按相似度排序并返回
        results.sort(key=lambda x: x['similarity'], reverse=True)
        return results[:top_k]

    def _deep_retrieve(self, query: str, top_k: int) -> List[Dict]:
        """深度检索 - 针对复杂问题"""
        # 结合TF-IDF和嵌入检索
        ...
        for result in tfidf_results:
            ...
            if doc_id not in all_results:
                ...
                all_results[doc_id]['combined_score'] = result['similarity'] * 0.4

        # 添加嵌入结果
        for result in embedding_results:
            doc_id = result['doc_id']
            if doc_id in all_results:
                all_results[doc_id]['combined_score'] += result['similarity'] * 0.6
            else:
                all_results[doc_id] = result
                all_results[doc_id]['combined_score'] = result['similarity'] * 0.6
    ...
    def predict_with_enhanced_rag(self, question: str) -> Tuple[str, float, List[Dict]]:
        ...
        # 使用自适应检索相关文档
        similar_docs = self.adaptive_retrieve(question, top_k=5)
